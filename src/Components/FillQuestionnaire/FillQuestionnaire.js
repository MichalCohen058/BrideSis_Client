import "../FillQuestionnaire/FillQuestionnaire.css"
export default function FillQuestionnaire(){
    return(<>
     <form>
        <label>:שם פרטי</label>
        <br/>
        <input type="text" />
        <br/>
        <br/>
        <label>:שם משפחה</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:גיל</label>
        <br/>
        <input type="number"/>
        <br/>
        <br/>
        <label>:מצב משפחתי</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:תאריך לידה</label>
        <br/>
        <input type="date"/>
        <br/>
        <br/>
        <label>:עיר</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:טלפון</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:תכונות אופי</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:גוון עור</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:גובה</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:מבנה גוף</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:מצב בריאותי</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:מצב כלכלי</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:סגנון לבוש</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:מראה כללי</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:מגזר</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:תמונה</label>
        <br/>
        <input type="img"/>
        <br/>
        <br/>
        <label>:שם ישיבה</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:עיסוק כיום</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:שם האב</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:שם האם</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:עיסוק האב</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:עיסוק האם</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:מוצא האב</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:מוצא האם</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:מספר אחים ואחיות</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:שיטה הלכתית</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <h2>ציפיות מבת הזוג</h2>
        <br/>
        <br/>
        <label>:שיוך מגזרי</label>
        <br/>
        <input type="text"/>  <br/>
        <br/>
        <label>:מראה כללי</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:תכונות אופי</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:ארץ מוצא צועדף</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:לא ממוצא</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:כיסוי ראש</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:מגיל</label>
        <br/>
        <input type="number"/>
        <br/>
        <br/>
        <label>:עד גיל</label>
        <br/>
        <input type="number"/>
        <br/>
        <br/>
        <label>:מגובה</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:עד גובה</label>
        <br/>
        <input type="text"/>
        <br/>
        <br/>
        <label>:פלאפון כשר</label>
        <br/>
        <input type="checkbox"/>
        <br/>
        <br/>
        <label>:רישיון</label>
        <br/>
        <input type="checkbox"/>
        <br/>
        <br/>
        <label>:מעשן</label>
        <br/>
        <input type="checkbox"/>
        
    </form>
    </>)
}